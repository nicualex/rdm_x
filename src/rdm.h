...
std::vector<uint8_t>
BuildRDMPacket(uint64_t destUID, uint64_t srcUID, uint8_t transNum,
               uint8_t portOrResponseType, uint8_t msgCount, uint16_t subDevice,
...
//    Overloaded for both driver types.
...
RDMResponse RDMGetCommand(EnttecPro &pro, uint64_t srcUID, uint64_t destUID,
                          uint16_t pid, const uint8_t *paramData = nullptr,
                          uint8_t paramLen = 0);
